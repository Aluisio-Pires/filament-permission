...
## Add The Trait
First, add the `AluisioPires\Permission\Traits\HasRoles` trait to your `User` model(s):
...
The `role` and `withoutRole` scopes can accept a string, a `\AluisioPires\Permission\Models\Role` object or an `\Illuminate\Support\Collection` object.
...
The scope can accept a string, a `\AluisioPires\Permission\Models\Permission` object or an `\Illuminate\Support\Collection` object.
